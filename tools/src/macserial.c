...
// confgen change
int serial_main(int argc, char *argv[], char *serial_result_sn, char *serial_result_mlb) {
  ...
  if (mode == MODE_SYSTEM_INFO) {
    ...
    for (int32_t i = 0; i < limit; i++) {
      ...
      if (get_serial(&tmp)) {
        ...
        // confgen change begin
        sprintf(serial_result_sn, "%3s%s%s%s%s", tmp.country, tmp.year, tmp.week, tmp.line, tmp.model);
        strncpy(serial_result_mlb, mlb, MLB_MAX_SIZE);
        // confgen change end
      }
    }
  ...
  return EXIT_SUCCESS; // confgen change
}
